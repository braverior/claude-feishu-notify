...
interface Config {
  notifyUserId: string; // default user to notify (open_id)
  larkCliBin: string;
}
...
function startEventListener(): void {
  ...
  eventProcess.on("exit", (code) => {
    console.error(
      `[feishu-bridge] Event listener exited with code ${code}, restarting in 5s...`
    );
    eventProcess = null;
...
server.tool(
  "feishu_send",
  "Send a message to a Feishu user or chat. Use this to notify the user about task progress, ask questions, or share results.",
  {
    content: z
      .string()
    ...
    user_id: z
      ...
      .optional()
      .describe(
        "Target user open_id (ou_xxx). Falls back to FEISHU_NOTIFY_USER_ID if not provided."
      ),
    chat_id: z
      .string()
      .optional()
...
server.tool(
  "feishu_inbox",
  "Check for new messages sent by the user via the Feishu bot. Call this periodically to see if the user has sent new instructions or feedback through Feishu.",
  {
    clear: z
      .boolean()
      .default(true)
...
server.tool(
  "feishu_reply",
  "Reply to a specific Feishu message. Use when you want to respond directly to a message the user sent via the bot.",
  {
  ...
  async ({ message_id, content }) => {
    try {
      ...
      return {
        content: [
          { type: "text" as const, text: result.trim() || "Reply sent." },
        ],
      };
    } catch (err: unknown) {
      const error = err as { stderr?: string; message?: string };
...
server.tool(
  "feishu_status",
  "Check the Feishu bridge status: event listener state, message queue size, and configuration.",
  {},
  async () => {
    const status = {
